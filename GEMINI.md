...
- **Directory Standard**: Maintain candidate-isolated output folders under `output/candidates/<candidate-slug>/<run-id>/` with subfolders `input/`, `scratch/`, `reports/`, `salary/`, `cv/`, `portfolio/`, `interview/`, `application/`, `platform/`. Do not write output files to the root directory.
